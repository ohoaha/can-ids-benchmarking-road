...
def make_can_df(log_filepath):
    ...
    #print(can_df)
    return can_df[can_df.aid<=0x700].reset_index(drop=True)
...
def payload_match(injection_payload, message_data):
    for i in range(len(injection_payload)):
        if injection_payload[i] != "X" and injection_payload[i] != message_data[i]:
            return False
    return True
...
def add_actual_attack_col(df, intervals, aid, payload):
    ...
    if aid != "XXX":
        df['actual_attack'] = df.time.apply(lambda x: sum(x >= intvl[0]  and x <= intvl[1] for intvl in intervals ) >= 1) & (df.aid == aid) & df.data.apply(lambda x: payload_match(payload, x))
